...
# ------------------------------------------------------------------------------
# function: nedc_load_parameters_lbl
#
# arguments:
#   pfile: .lbl file
#
...
# is stored as one entry (a list) in the dictionary.
#
def nedc_load_parameters_lbl(pfile_a):
    # declare local variables
    #
    values = OrderedDict()
    keyword_upcase = KEYWORD_MONTAGE.upper()

    # default channel selection and match mode
    values['channel_selection'] = '(null)'
    values['match_mode'] = 'exact'
    ...
    for line in fp:
        ...
        # note that we exit if we hit the end of the parameter block
        #
        elif (flag_pblock == True) and (len(str) == 0):
            fp.close()
            break
        ...
        # by splitting and assigning to a dictionary
        #
        elif (str.startswith('montage') == True) and ('=' in str):
            flag_pblock = True
            parts = str.split(DELIM_EQUAL)
